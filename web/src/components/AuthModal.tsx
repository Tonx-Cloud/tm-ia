...
export function AuthModal({ locale = 'en', open, onClose, onAuth, onSkipDemo }: Props) {
  ...
  // ==========================================================================
  // SOCIAL AUTH HANDLER - Using Supabase Auth
  // ==========================================================================
  const handleSocialAuth = async (provider: 'google' | 'apple') => {
    if (provider === 'google') {
      ...
      try {
        // Use Supabase OAuth - this redirects to Google
        await signInWithGoogle()
        // The redirect will happen automatically
        // After auth, Supabase redirects to /auth/callback with the session
      } catch (err) {
        setError((err as Error).message || 'Erro ao conectar com Google. Tente novamente.')
        setLoading(false)
      }
      return
    }
